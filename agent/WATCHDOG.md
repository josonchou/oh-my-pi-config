...
- 审查执行纪律：破坏性操作须先获确认；与项目有关的 Node.js 命令须优先遵循项目显式工具链版本，只有项目无版本约束或命令与项目无关时，`node` / `npx` 才使用 `mise x node@20 --` 固定版本；提交信息须使用中文 Conventional Commits 格式；未获用户明确要求不得主动执行浏览器自动化测试。
